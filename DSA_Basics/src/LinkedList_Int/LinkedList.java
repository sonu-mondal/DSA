...
public class LinkedList {
	private Node head;
	private Node tail;// advantage of taking tail is that when we need to add an element at the last
						// of LL
	// then that we can do in the O(1) complexity i.e constant time , if tail is not
	// there then we need
...
//	algo: create the node with the given data
	...
	// else point the earlier nodeBefores next element as the next element of the
	// node.
	...
	public void insertUsingRecursion(int data, int index) {
		// head=... because we need to update the head if ant element is inserted at 0th
		// position
	...
	private Node insertUsingRecursion(int data, int index, Node node) {
		if (index == 0) {// index=0 means if we moved to position where we need to insert the data
			// otherwise we will move to next index and decrement the index value till it
			// become 0
...
// add to our list by making tails next point to that node
	...
	// concept is that when fast==slow then we keep fast as it is , we move slow by
	// one position
	// and will keep moving slow untill it again become equal to fast and will keep
	// incrementing the
...
//			Output: false
	...
	// Reorder list
	// concept is: if we take two pinter one at first and one at last node and keep
	// adding in sequence
	// first-last then increment first and decrement last but here we can go back
	// node
//so concept is we find mid element and from mid to last we reverse the list
		...
		public void reorderList(Node head) {
			        ...
			        //middle will hold head second 
			        Node middle=findMid(head);

			        Node headSecond=reverseList(middle);//middle acting as head for 2nd    part of list
		...
		//deleting the mid element of the ll
